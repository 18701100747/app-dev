			...
			<uni-card @tap="onClick(data)" :isShadow="false" :border="false" margin="0 20px 10px" padding="10px"
				...
				<template v-for="(config, index) in newColumnConfigs">
					<view class="column-line">
						<text class="label-title" v-if="config.showLabel&&!config.mainField">{{ config.title }}1：</text>
...
import {
	...
	onMounted(() => { 
		...
		console.log('newColumnConfigs', newColumnConfigs.value, datasourceRef.value)
